...
const AccordionItem = ({role, company, description, range, skills}: AccordionItemProps) => {
  ...
  return (
    <AccordionPrimitive.Item value={company}>
      ...
      >
        <ChevronDownIcon
          className="text-gray-10 group-data-[state=open]:-rotate-90 transition-transform duration-150 mr-2 group-hover:text-gray-12 group-focus-visible:text-gray-12 group-data-[state=open]:text-gray-1 -translate-y-px w-3 h-3"
          aria-hidden={true}
        />
        ...
        <span>{company}</span>
        <span
          className="hidden sm:block ml-auto text-gray-10 text-sm group-hover:text-gray-12 group-focus-visible:text-gray-12 group-data-[state=open]:text-gray-1">
          {range}
        </span>
        <span
          className="sm:hidden ml-auto text-gray-10 text-sm group-hover:text-gray-12 group-focus-visible:text-gray-12 group-data-[state=open]:text-gray-1">
          {shortRange}
        </span>
      </AccordionPrimitive.Trigger>
...
const Accordion = ({className, children}: { children: ReactNode, className: string }) => {
  return (
